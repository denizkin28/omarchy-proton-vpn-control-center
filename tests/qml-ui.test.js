...
test("enhanced server discovery remains read-only and connects through the CLI", () => {
  ...
  assert.match(panel, /onWheel: function\(wheel\)/)
  assert.match(panel, /wheel\.modifiers & Qt\.ControlModifier/)
  assert.match(panel, /drag\.target: serverMap\.zoom > 1 \? mapContent/)
  assert.match(panel, /Ctrl\+scroll to zoom/)
  assert.match(panel, /assets\/world-map\.svg/)
